...
def get_bot_response():
    ...
    if s and s.lower() == "q":
        ...
        session["ended"] = True
        return (
            "Thank you for using Robo-Doc.<br>"
            "This consultation has been stopped.<br>"
            "Please refresh the page to start a new consultation."
        )
    if session.get("ended"):
        return "This consultation has already ended. Please refresh the page to start again."
    s = request.args.get('msg')
    if "step" in session:
        if session["step"] == "Q_C":
            name = session["name"]
            ...
            session.clear()
            if s == "q":
                "Thank you for using our website " + session["title"] + " " + name
            else:
                session["step"] = "FS"
                session["name"] = name
                session["age"] = age
    ...
    if session['step'] == 'PD':
        ...
        if not diseases:
            session['step'] = "END"
            return "I couldn’t find a disease matching these symptoms. Can you describe more symptoms?"
    ...
    if session['step'] == "PREDICT":
        pred = knn_clf.predict(OHV(session["all"], all_symp_col))
        session["result"] = pred[0]   # convert ndarray → string
        session['step'] = "END"
    ...
    if session['step'] == "END":
        ...
        if result is not None:
            if result != session.get("testpred"):
                session['step'] = "Q_C"
                return (
    ...
    if session['step'] == "FINAL":
        session['step'] = "BYE"
        return "Your diagnosis was perfectly completed. Do you need another medical consultation? (yes / no)"
    ...
    if session['step'] == "BYE":
        if s.lower() == "yes":
            ...
            session["ended"] = True
            return (
                "This consultation has ended.<br>"
                "Please refresh the page to start a new consultation."
            )
        else:
            session.clear()
