...
X_train = df_tr.iloc[:, :-1].values  # All symptoms (features)
y_train = df_tr.iloc[:, -1].values   # Disease (label)
...
def OHV(symptoms, all_symp_col):
    if symptoms is None:
        symptoms = []  # Avoid errors if session["all"] is None
    processed_symptoms = [symptom.replace(" ", "_") for symptom in symptoms]
    # Ensure feature vector has correct format
    feature_vector = [1 if symptom in processed_symptoms else 0 for symptom in all_symp_col]
    return [feature_vector]  # sklearn expects a 2D array
...
# IV- Prediction Model (Naive Bayes)
# load model
nb_clf = joblib.load('model/naive_bayes.pkl')
...
def get_bot_response():
    ...
    if session['step'] == "FS":
        ...
        session['FSY'] = temp  # info of 1st symptome
        session['step'] = "SS"  # second symptome
        if sim1 == 1:
    ...
    if session['step'] == "SS":
        ...
        session['SSY'] = temp  # info of 2nd symptome(sym,sim,psym)
    ...
    if session['step'] == "semantic":
        temp = session["FSY"]  # retrieve info from the 1st symptom
        sym1 = temp[0]
        sim1 = temp[1]
        temp = session["SSY"]  # retrieve info from the 2nd symptom
        sym2 = temp[0]
        sim2 = temp[1]
        if sim1 == 0 or sim2 == 0:
    ...
    if session['step'] == "PREDICT":
        result = nb_clf.predict(OHV(session["all"], all_symp_col))
        session['step'] = "END"
    if session['step'] == "END":
    ...
    if session['step'] == "BYE":
        ...
        if s.lower() == "yes":
            ...
            return "THANKS Mr/Ms " + name + " for using me for more information please contact <b> +919999999999</b>"
